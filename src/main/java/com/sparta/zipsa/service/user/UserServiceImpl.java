...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity signup(SignupRequestDto signupRequestDto, MultipartFile multipartFile) {
        ...
        fileServiceImpl.constructor(); //저장경로 없으면 만들어주려나 포스트맨 돌려보고 싶은데..
        fileServiceImpl.upload(multipartFile, fileName); //업로드할 사진, uuid 적용한 파일이름
    ...
    public ResponseEntity delete(DeleteRequestDto deleteRequestDto, User user) {
        ...
        if (user.getRole().equals(UserRoleEnum.ADMIN)
            ...
            fileServiceImpl.deleteFile(user.getUserImage()); // 이렇게 하면 파일도 삭제되려나.. 포스트맨으로 돌려보고 싶은데..
